...
# Check which keys are available (use print(calib_data.files) to see them)
camera_matrix = calib_data['mtx']   # or calib_data['mtx'] if that’s your key
dist_coeff = calib_data['dist']           # or calib_data['dist']
...
def capture_frame():
    ...
    while running:
        ...
        #undistort the image
        if 'dist_coeff' in calib_data:
            dist_coeff = calib_data['dist_coeff']
            frame = cv2.undistort(frame, camera_matrix, dist_coeff, None, camera_matrix)
...
def process_video():
    ...
    while running:
        ...
        with torch.no_grad():
            ...
            #added for static calibration
            depth_map = depth_map / 2.8
        ...
        for bbox in bbox_data:
            ...
            X_estimate = (center_x - cx) * depth_value / fx
            Y_estimate = (center_y - cy) * depth_value / fy
            Z_estimate = depth_value  # Already metric
            print(f"3D Coordinates: X={X_estimate:.2f}, Y={Y_estimate:.2f}, Z={Z_estimate:.2f}")
        if bbox_data:
